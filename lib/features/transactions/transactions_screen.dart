...
class _TransactionsScreenState extends ConsumerState<TransactionsScreen> {
  ...
  final List<String> _networks = ['ERC20 (Ethereum)', 'TRC20 (Tron)', 'BEP20 (BNB Smart Chain)', 'SOL (Solana)'];
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Manage terminal capital & transfers', style: DexTypography.bodySmall).animate().fade(delay: 100.ms),
                  ...
                  children: [
                    Expanded(
                      child: GlowButton(
                        ...
                        onPressed: () => setState(() { _showDeposit = true; _showWithdraw = false; _depositStep = 1; }),
                      ),
                    ),
          ...
          Expanded(
            child: txAsync.when(
              data: (txns) {
                ...
                if (filtered.isEmpty) {
                  return const EmptyStateWidget(
                    ...
                    title: 'No Logs Found',
                    subtitle: 'Terminal execution history is empty for the selected filter.',
              ...
              error: (_, __) => const Center(child: Text('Failed to load transaction nodes')),
            ),
          ),
        ],
      ),
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text('INITIALIZE DEPOSIT NODE', style: GoogleFonts.orbitron(fontSize: 10, fontWeight: FontWeight.w900, color: DexColors.successGlow, letterSpacing: 1.0)),
              const Spacer(),
              GestureDetector(
                onTap: _resetForms,
          ...
          Row(
            children: [
              Expanded(child: Container(height: 4, color: _depositStep >= 1 ? DexColors.success : DexColors.border)),
              const SizedBox(width: 4),
              Expanded(child: Container(height: 4, color: _depositStep >= 2 ? DexColors.success : DexColors.border)),
              const SizedBox(width: 4),
              Expanded(child: Container(height: 4, color: _depositStep >= 3 ? DexColors.success : DexColors.border)),
            ],
          ),
          ...
          if (_depositStep == 1) ...[
            Text('Select Target Network', style: DexTypography.bodyMedium.copyWith(color: Colors.white, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            ..._networks.map((n) => GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: _selectedNetwork == n ? DexColors.success : DexColors.border),
                  color: _selectedNetwork == n ? DexColors.success.withValues(alpha: 0.1) : Colors.transparent,
                ),
                child: Row(
                  children: [
                    Icon(Icons.hub_rounded, size: 18, color: _selectedNetwork == n ? DexColors.success : DexColors.textMuted),
                    const SizedBox(width: 12),
                    Text(n, style: TextStyle(color: _selectedNetwork == n ? Colors.white : DexColors.textSecondary, fontWeight: FontWeight.w600)),
                    const Spacer(),
                    if (_selectedNetwork == n) const Icon(Icons.check_circle_rounded, size: 18, color: DexColors.success),
                  ],
                ),
              ),
            )),
            const SizedBox(height: 16),
            GlowButton(label: 'Continue', onPressed: () => setState(() => _depositStep = 2), width: double.infinity, isPrimary: true, icon: Icons.arrow_forward_rounded),
          ],

          // Step 2: Amount Entry
          if (_depositStep == 2) ...[
            Text('Specify Transfer Volume', style: DexTypography.bodyMedium.copyWith(color: Colors.white, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Container(
              ...
              child: TextField(
                ...
                style: GoogleFonts.jetBrainsMono(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white),
                decoration: InputDecoration(
                  hintText: '0.00',
                  hintStyle: GoogleFonts.jetBrainsMono(fontSize: 24, color: Colors.white30),
                  prefixText: '\$ ',
                  prefixStyle: GoogleFonts.jetBrainsMono(fontSize: 24, color: DexColors.success),
            ...
            Row(
              children: [
                Expanded(child: GlowButton(label: 'Back', onPressed: () => setState(() => _depositStep = 1), isPrimary: false)),
                const SizedBox(width: 12),
                Expanded(
                  child: GlowButton(
                    label: 'Continue',
                    onPressed: () {
                      ...
                      if (val != null && val > 0) {
                        ...
                        DexToast.showPushNotification(context, title: 'Error', body: 'Invalid deposit volume');
                      }
                    },
                  ),
                ),
          ...
          if (_depositStep == 3) ...[
            Text('Initiate Node Sync', style: DexTypography.bodyMedium.copyWith(color: Colors.white, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Container(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      Text('\$${_amountCtrl.text}', style: GoogleFonts.jetBrainsMono(color: Colors.white, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text(_selectedNetwork, style: GoogleFonts.jetBrainsMono(color: Colors.white, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ],
              ),
            ...
            Row(
              children: [
                Expanded(child: GlowButton(label: 'Back', onPressed: () => setState(() => _depositStep = 2), isPrimary: false)),
                const SizedBox(width: 12),
                Expanded(
                  child: GlowButton(
    ...
    return GlassCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text('WITHDRAWAL PROTOCOL', style: GoogleFonts.orbitron(fontSize: 10, fontWeight: FontWeight.w900, color: DexColors.errorGlow, letterSpacing: 1.0)),
              const Spacer(),
              GestureDetector(
                onTap: _resetForms,
                child: const Icon(Icons.close_rounded, size: 20, color: DexColors.textMuted),
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: GoogleFonts.jetBrainsMono(fontSize: 24, color: Colors.white30),
                prefixText: '\$ ',
                prefixStyle: GoogleFonts.jetBrainsMono(fontSize: 24, color: DexColors.error),
                border: InputBorder.none,
              ),
            ),
          ),
          ...
          Container(
            ...
            child: TextField(
              ...
              decoration: const InputDecoration(
                hintText: '0x...',
                border: InputBorder.none,
                icon: Icon(Icons.account_balance_wallet_rounded, color: DexColors.textMuted, size: 20),
              ),
            ),
          ),
          const SizedBox(height: 24),
    ...
    try {
      ...
      if (_showDeposit) {
        ...
        if (_walletCtrl.text.isEmpty) {
          DexToast.showPushNotification(context, title: 'Error', body: 'Wallet address required');
          return;
        }
      ...
      if (mounted) {
        DexToast.showPushNotification(context, title: 'Syncing...', body: 'Transfer Protocol Initiated. Syncing terminal...');
        showDialog(
          context: context,
